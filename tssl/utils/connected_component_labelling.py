...
if __name__ == "__main__":
    ...
    cca = TrivialDetector()
    num_labels, labels = cca.cca(x, S)
    
    fig, axs = plt.subplots(1,3,figsize=(16,8))
    axs[0].plot(x, y, '.')
    axs[0].plot([min(x), max(x)], [0, 0], '-')
    axs[1].plot(x, S, 'o')
    axs[2].plot(x, labels, 'o')
    plt.show()
    """
    """
    #2D check
    from tssl.oracles.mogp_oracle import MOGPOracle
    ...
    from tssl.utils.utils import create_grid
    kernel_list = [
            KernelFactory.build(BasicMatern52Config(input_dimension=2, base_lengthscale=0.8, base_variance=0.9)),
            KernelFactory.build(BasicMatern52Config(input_dimension=2, base_lengthscale=1.2, base_variance=1.1))
        ]
    W = np.array([[0.1, 0.8],
                [-0.1, 0.9]])
    oracle = MOGPOracle(kernel_list, W, 0.01, 0)
    
    grid, function = oracle._gp_sample(-2, 2, 50)
    S = function[:,0,None]>=0

    perm = np.random.permutation(2500)
    cca = TwoPass()
    num_labels, labels = cca.cca(grid[perm], S[perm])
    labels = labels[np.argsort(perm)]
    
    perm = np.random.permutation(2500)
    cca_test = TwoPassHighDim()
    test_num_labels, test_labels = cca_test.cca(grid[perm], S[perm])
    test_labels = test_labels[np.argsort(perm)]

    print(num_labels, test_num_labels)
    #assert test_num_labels == num_labels

    fig = plt.figure(figsize=(20,8))
    ax = fig.add_subplot(1,4,1)
    contour = ax.tricontourf(grid[:,0], grid[:,1], function[:,0], levels=np.linspace(-2, 2, 50), cmap='seismic')
    fig.colorbar(contour, ax=ax)
    ax = fig.add_subplot(1,4,2)
    contour = ax.tricontourf(grid[:,0], grid[:,1], S.astype(int)[:,0], levels=[-0.5, 0.5, 1.5, 2.5], cmap='YlGn')
    fig.colorbar(contour, ax=ax)
    ax = fig.add_subplot(1,4,3)
    for l in range(1, num_labels+1):
        ax.plot(grid[labels[:,0]==l,0], grid[labels[:,0]==l,1], '.', color=f'C{l}')
    ax = fig.add_subplot(1,4,4)
    for l in range(1, test_num_labels+1):
        ax.plot(grid[test_labels[:,0]==l,0], grid[test_labels[:,0]==l,1], '.', color=f'C{l}')
    plt.show()
    """
