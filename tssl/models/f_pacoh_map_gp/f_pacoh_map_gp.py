...
from tssl.models.f_pacoh_map_gp.models import (
...
from tssl.models.f_pacoh_map_gp.util import _handle_input_dimensionality, DummyLRScheduler
from tssl.models.f_pacoh_map_gp.abstract import RegressionModelMetaLearned
...
class FPACOH_MAP_GP(RegressionModelMetaLearned):
    ...
    def __check_domain_bound(self, bound: Union[float, np.ndarray]):
        if hasattr(bound, '__len__'):
            assert len(bound) == self.input_dim
            return bound
        else:
...
if __name__ == "__main__":
    from tssl.oracles import BraninHoo, GPOracle1D
