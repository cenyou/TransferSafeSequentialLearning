...
see folder ./meta_bo/models/models.py
...
from tssl.models.f_pacoh_map_gp.util import find_root_by_bounding
...
class PredictedScaleKernel(Kernel):
    """Scale kernel whose outputscale is dependent on X. It takes the last slice
       of the last dimention and treats it as raw sigma. The resulting kernel
       reads as:
       $$k(x,x') = \sqrt(\sigma(x) * \sigma(x')) * k_base(x, x')$$
    """

    def __init__(self, base_kernel: torch.nn.Module, outputscale_constraint=None, **kwargs):
        super().__init__(**kwargs)
        self.outputscale_constraint = Positive() if outputscale_constraint is None else outputscale_constraint
    ...
    def forward(self, x1, x2, last_dim_is_batch=False, diag=False, **params):
        x1, predicted_scale_raw1 = x1[:, :-1], x1[:, -1]
        x2, predicted_scale_raw2 = x2[:, :-1], x2[:, -1]
        orig_output = self.base_kernel.forward(x1, x2, diag=diag, last_dim_is_batch=last_dim_is_batch, **params)

        o1 = self.outputscale_constraint.transform(predicted_scale_raw1)
        o2 = self.outputscale_constraint.transform(predicted_scale_raw2)
        if diag:
            output_scale_matrix = torch.sqrt(o1 * o2)
        else:
            output_scale_matrix = torch.sqrt(o2 * o1[:, None])

        assert output_scale_matrix.shape == orig_output.shape
        return output_scale_matrix * orig_output
...
class LearnedGPRegressionModel(gpytorch.models.ExactGP):
    ...
    def __init__(self, train_x: torch.Tensor, train_y: torch.Tensor, likelihood: torch.nn.Module,
                 learned_kernel: Optional[torch.nn.Module] = None,
                 learned_mean: Optional[torch.nn.Module] = None,
