...
from tssl.utils.utils import create_grid_multi_bounds, check1Dlist, row_wise_compare, filter_nan
from tssl.models.base_model import BaseModel
from tssl.oracles.base_oracle import BaseOracle
from tssl.oracles.context_interface import ContextSupport

class GPOracleFromData(BaseOracle, ContextSupport):
    def __init__(
        self,
        gp_model: BaseModel,
        x_data: np.ndarray,
    ...
    def get_contextual_box_bound(self): # this method is used in tssl.pools.engine_pool
        a, b = self.get_box_bounds()
        use_context, context_idx, context_values = self.get_context_status(return_idx=True, return_values=True)
        if use_context:
            a[context_idx] = context_values
